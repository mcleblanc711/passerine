...
def collect():
    ...
    return Observation(source_id='whiskeyjack', instance_id='minibench:' + hashlib.sha256(str(Path(os.environ['PASSERINE_WJ_DB']).resolve()).encode()).hexdigest()[:16], origin='real', source_observed_at=data.pop('source_observed_at'), heartbeat_at=data.pop('heartbeat_at'), heartbeat_note=data.pop('heartbeat_note'), provenance='Matching checkout connect_readonly + assemble_show; recorded worker heartbeat; MiniBench namespace', coverage='Full recorded MiniBench history; official platform scores unavailable. Resolution ingestion scheduled every six hours. Cup excluded (withdrawn).', events=data.pop('events'), data=data)
