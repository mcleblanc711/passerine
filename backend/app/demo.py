...
def collect(source):
    ...
    if source == 'whiskeyjack':
        ...
        for record, title, probability in zip(records, ['Will the synthetic launch occur by Friday?', 'Will the synthetic station report rain?'], [.7, .4]):
            record.update(title=title, group_title=None,
                          forecast=dict(version=1, generated_at=AT, as_of=AT, kind='binary', unit=None,
                                        rows=[dict(label='Yes', value=probability), dict(label='No', value=1-probability)]),
                          community=dict(status='unavailable', observed_at=None, reason='Synthetic example of missing community evidence.'))
